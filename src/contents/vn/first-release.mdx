...
- $x_\{target\}$ là hình ảnh neo (anchor image) đại diện cho $c_\{target\}$.
...
```math
\min_{\delta} [ \mathcal{L}_{style}(\Phi(x+\delta), \Phi(S_{target})) + \alpha \mathcal{L}_{content}(\Psi(x+\delta), \Psi(x)) ]
```
...
1.  **Về mặt Thuật toán**: Hope-AD sử dụng chung thuật toán lõi (Projected Gradient Descent) với bản chính gốc của ĐH Chicago (Glaze/Nightshade Team). Nên hiệu quả tấn công là tương đương.
2.  **Về mặt Thực tế**:
...
- **Towards Deep Learning Models Resistant to Adversarial Attacks**: [Madry et al., ICLR 2018](https://arxiv.org/abs/1706.06083)
  - _Chi tiết tham khảo_: **Mục 2 (Vấn đề Điểm yên ngựa)**, tr. 2-4. Định nghĩa thuật toán Projected Gradient Descent (PGD), là bộ giải thuật toán cốt lõi cho Hope-AD.
