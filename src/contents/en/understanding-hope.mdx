...
Glaze minimizes the distance between the protected image's style embedding and a target style $S_{target}$, while preserving the original content $x$.
...
Where $\Phi(\cdot)$ extracts style features (e.g., via Gram matrices or specialized style encoders).

#### 2. Nightshade: Concept Poisoning
Nightshade targets semantic alignment by shifting the CLIP visual embedding of $x$ towards a completely unrelated concept $x_{target}$.

$$
\min_{\delta} || E(x + \delta) - E(x_{target}) ||_2^2 + \lambda \cdot \text{PerceptualLoss}(x, x + \delta)
$$
Subject to: $||\delta||_{p} \leq \epsilon$
